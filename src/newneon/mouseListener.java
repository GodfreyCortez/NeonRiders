...
public class mouseListener extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
        mx = e.getX();
        my = e.getY();
        if (Game_Loop.state == State.MENU) //depending on the state we are in we want to ensure we that the clicks do not lead to the wrong state
        {
            ...
            if (mx >= 192 && mx <= 1439) {
                ...
                } else if (my >= 762 && my <= 851) //these are the y boundaries of the main menu button
                {
                    ...
                    Game_Loop.state = State.MENU;
                }
            }
        } else if (Game_Loop.state == State.MULTIPLAYER_SELECTION) {
        ...
        } else if (Game_Loop.state == State.INSTRUCTIONS) //Will display the instructions and goal of the game
        {
            if (mx >= 192 && mx <= 1439) {
                if (my >= 220 && my <= 275) {
                    Game_Loop.state = State.MENU;
        ...
        } else if (Game_Loop.state == State.GAME) //if we are in the game state we will implement a pause
        {
